...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["id"]) && isset($_POST["tantsija_hinne3"])) {
    $kask = $yhendus->prepare("UPDATE `Tantsupaarid` SET `Hinne3`=? WHERE `TantsijaID`=?");
    $kask->bind_param("ii", $_POST["tantsija_hinne3"], $_POST["id"]);
    $kask->execute();
}
